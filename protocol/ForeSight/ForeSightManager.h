...
#include "protocol/ForeSight/ForeSightExecutor.h"
...
template <class Workload> class ForeSightManager : public aria::Manager {
public:
  ...
  ForeSightManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
  ...
  void coordinator_start() override {
    ...
    while (!stopFlag.load()) {
      ...
      signal_worker(ExecutorStatus::ForeSight_READ);
      ...
      wait_all_workers_finish();
      // wait for all machines until they finish the ForeSight_READ phase.
      wait4_ack();

      // Allow each worker to commit transactions
      n_started_workers.store(0);
      n_completed_workers.store(0);
      signal_worker(ExecutorStatus::ForeSight_COMMIT);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      // clean batch now
      cleanup_batch();
      // wait for all machines until they finish the ForeSight_COMMIT phase.
      // this can be skipped
      // wait4_ack();
      ...
      if (context.calvin_fallback==true){
        ...
        n_started_workers.store(0);
        n_completed_workers.store(0);
        signal_worker(ExecutorStatus::ForeSight_Fallback_Prepare);
        wait_all_workers_start();
        wait_all_workers_finish();
        broadcast_stop();
        wait4_stop(n_coordinators - 1);
        ...
        signal_worker(ExecutorStatus::ForeSight_Fallback);
        wait_all_workers_start();
        wait_all_workers_finish();
        broadcast_stop();
        wait4_stop(n_coordinators - 1);
        ...
        // prepare transactions for bohm and clear the metadata
        n_started_workers.store(0);
        n_completed_workers.store(0);
        signal_worker(ExecutorStatus::ForeSight_Fallback_Prepare);
        wait_all_workers_start();
        ...
        set_worker_status(ExecutorStatus::STOP);
        wait_all_workers_finish();
        // wait for all machines until they finish the ForeSight_Fallback_Prepare phase.
        wait4_ack();

        // Allow each worker to insert write sets
        n_started_workers.store(0);
        n_completed_workers.store(0);
        signal_worker(ExecutorStatus::ForeSight_Fallback_Insert);
        wait_all_workers_start();
        wait_all_workers_finish();
        broadcast_stop();
        wait4_stop(n_coordinators - 1);
        ...
        set_worker_status(ExecutorStatus::STOP);
        wait_all_workers_finish();
        // wait for all machines until they finish the ForeSight_Fallback_Insert phase.
        wait4_ack();

        // Allow each worker to run transactions
        n_started_workers.store(0);
        n_completed_workers.store(0);
        signal_worker(ExecutorStatus::ForeSight_Fallback_Execute);
        wait_all_workers_start();
        wait_all_workers_finish();
        broadcast_stop();
        wait4_stop(n_coordinators - 1);
        ...
        set_worker_status(ExecutorStatus::STOP);
        wait_all_workers_finish();
        // wait for all machines until they finish the ForeSight_Fallback_Execute phase.
        wait4_ack();
      }
      ...
      n_started_workers.store(0);
      n_completed_workers.store(0);
      signal_worker(ExecutorStatus::ForeSight_GC);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
  ...
  void non_coordinator_start() override {
    ...
    for (;;) {
      ...
      DCHECK(status == ExecutorStatus::ForeSight_READ);
      // the coordinator on each machine first moves the aborted transactions
      // from the last batch earlier to the next batch and set remaining
      // transaction slots to null.
      ...
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::ForeSight_READ);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      status = wait4_signal();
      DCHECK(status == ExecutorStatus::ForeSight_COMMIT);
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::ForeSight_COMMIT);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      status = wait4_signal();
      DCHECK(status == ExecutorStatus::ForeSight_Fallback_Prepare);
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::ForeSight_Fallback_Prepare);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      status = wait4_signal();
      DCHECK(status == ExecutorStatus::ForeSight_Fallback_Insert);
      // Allow each worker to run transactions
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::ForeSight_Fallback_Insert);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      status = wait4_signal();
      DCHECK(status == ExecutorStatus::ForeSight_Fallback_Execute);
      // Allow each worker to run transactions
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::ForeSight_Fallback_Execute);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      status = wait4_signal();
      DCHECK(status == ExecutorStatus::ForeSight_GC);
      // Allow each worker to garbage collect
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::ForeSight_GC);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
      ...
      if (context.calvin_fallback==true){
        ...
        status = wait4_signal();
        DCHECK(status == ExecutorStatus::ForeSight_Fallback);
        n_started_workers.store(0);
        n_completed_workers.store(0);
        clear_lock_manager_status();
        set_worker_status(ExecutorStatus::ForeSight_Fallback);
        wait_all_workers_start();
        wait_all_workers_finish();
        broadcast_stop();
        wait4_stop(n_coordinators - 1);
  ...
  void add_worker(const std::shared_ptr<ForeSightExecutor<WorkloadType>>
...
public:
  ...
  std::vector<std::shared_ptr<ForeSightExecutor<WorkloadType>>> workers;
