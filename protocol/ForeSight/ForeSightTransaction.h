...
class ForeSightTransaction {

public:
  using MetaDataType = std::atomic<uint64_t>;

  ForeSightTransaction(std::size_t coordinator_id, std::size_t partition_id,
  ...
  virtual ~ForeSightTransaction() = default;
  ...
  void search_local_index(std::size_t table_id, std::size_t partition_id,
                          const KeyType &key, ValueType &value, std::size_t keyID) {
    if (execution_phase) {
      return;
    }

    ForeSightRWKey readKey;
  ...
  void update(std::size_t table_id, std::size_t partition_id,
              const KeyType &key, const ValueType &value, std::size_t keyID) {
    if (execution_phase) {
      return;
    }

    ForeSightRWKey writeKey;
    ...
    writeKey.set_keyID(keyID);

    add_to_write_set(writeKey);
  }

  std::size_t add_to_read_set(const ForeSightRWKey &key) {
    readSet.push_back(key);
    return readSet.size() - 1;
  }

  std::size_t add_to_write_set(const ForeSightRWKey &key) {
  ...
  void setup_process_requests_in_execution_phase() {
    ...
    process_requests = [this](std::size_t worker_id) {
      ...
      for (int i = int(readSet.size()) - 1; i >= 0; i--) {
        ...
        ForeSight_read_handler(readKey, id, i);
  ...
  void 
  fallback_setup_process_requests_in_execution_phase() {
    ...
    process_requests = [this](std::size_t worker_id) {
      ...
      if (pendingResponses > 0) {
        ...
        while (pendingResponses > 0) {
          fallback_remote_request_handler();
        }
      }
  ...
  setup_process_requests_in_fallback_phase(std::size_t n_lock_manager,
    ...
    process_requests = [this, n_lock_manager, n_worker,
      ...
      auto lock_manager_id = ForeSightHelper::worker_id_to_lock_manager_id(
...
public:
  ...
  bool abort_read_not_ready, abort_lock, abort_no_retry, abort_read_validation;
  bool distributed_transaction;
  bool relevant, run_in_aria;
  bool execution_phase;
  bool waw, war, raw, can_commited;
  ...
  std::function<void(ForeSightRWKey &, std::size_t, std::size_t)>
      ForeSight_read_handler;
  ...
  std::function<void()> fallback_message_flusher;
};
} // namespace aria