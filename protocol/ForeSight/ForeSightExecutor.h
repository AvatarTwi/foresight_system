...
#include "protocol/ForeSight/UncommittedTransactions.h"
...
public:
  ...
  void start() override {
    ...
    for (;;) {
      ...
      // LOG(INFO) << "ForeSightExecutor " << id << " starts ForeSight_READ phase. ";
      n_started_workers.fetch_add(1);
      // we find active coord and relevant transactions
      generate_transactions();
      read_snapshot();
      n_complete_workers.fetch_add(1);
      // wait to ForeSight_READ
      ...
      // wait till ForeSight_COMMIT
      // LOG(INFO) << "ForeSightExecutor " << id << " starts ForeSight_COMMIT phase. ";
      while (static_cast<ExecutorStatus>(worker_status.load()) !=
      ...
      if (context.calvin_fallback==true){
        // wait till ForeSight_Fallback_Prepare
        while (static_cast<ExecutorStatus>(worker_status.load()) !=
        ...
        // wait till ForeSight_Fallback_Prepare
        // LOG(INFO) << "ForeSightExecutor " << id << " starts ForeSight_Fallback_Prepare phase. ";
        while (static_cast<ExecutorStatus>(worker_status.load()) !=
               ExecutorStatus::ForeSight_Fallback_Prepare) {
          std::this_thread::yield();
        }
        ...
        // LOG(INFO) << "ForeSightExecutor " << id << " starts ForeSight_Fallback_Insert phase. ";
        while (static_cast<ExecutorStatus>(worker_status.load()) !=
        ...
        // LOG(INFO) << "ForeSightExecutor " << id << " starts ForeSight_Fallback_Execute phase. ";
        while (static_cast<ExecutorStatus>(worker_status.load()) !=
      ...
      // LOG(INFO) << "ForeSightExecutor " << id << " starts ForeSight_GC phase. ";
      while (static_cast<ExecutorStatus>(worker_status.load()) !=
  ...
  /*
   * We run the same batch of transactions in ForeSight for simplicity
   * Each node generates the same set of transactions.
   * */

  void generate_transactions() {
    const auto& uncommitted_transactions = UncommittedTransactionsConfig::get_uncommitted_transactions(context);
    // single node
    if (context.coordinator_num == 1) {
      for (auto i = id; i < transactions.size(); i += context.worker_num) {
        ...
        // check if the transaction is can_commited
        if (uncommitted_transactions.find(transactions[i]->id) !=
            uncommitted_transactions.end()) {
          transactions[i]->can_commited = false;
          transactions[i]->abort_lock = true;
        } else {
          transactions[i]->can_commited = true;
        }
  ...
  void read_snapshot() {
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      // 如果transactions[i]是未提交的事务，则跳过
      if (transactions[i]->can_commited==false) {
        continue;
      }
  ...
  //       continue;
  //     }
  //     if (transactions[i]->can_commited) {
  ...
  void commit_transactions() {
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      ...
      if (transactions[i]->waw&&transactions[i]->raw) {
  ...
  void prepare_fallback_input() {
    ...
    // otherwise, we analyse the read and write set.
    // 打印进入的log
    // LOG(INFO) << "prepare_fallback_input started. ";
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      ...
      // LOG(INFO) << "prepare_fallback_input: transaction " << transactions[i]->id;
  ...
  void insert_write_sets() {
    ...
    // 打印进入的log
    // LOG(INFO) << "insert_write_sets started. ";
    for (auto i = 0u; i < transactions.size(); i++) {
      // commit in aria
      if (transactions[i]->abort_lock == false) {
        continue;
  ...
  void run_transactions() {
    ...
    // 打印进入的log
    // LOG(INFO) << "run_transactions started. ";
    if (context.if_local) {
      for (auto i = id; i < transactions.size(); i += context.worker_num) {
