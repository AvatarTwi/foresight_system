...
public:
  ...
  // 用mvccmap_的最新版本更新map_
  virtual void garbage_collect_fallback(const void *key) =0;
  
  virtual std::tuple<MetaDataType *, void *> search_prev_fallback(const void *key,
    uint64_t version)=0;
  virtual void fallback_insert(const void *key, const void *value,
              uint64_t version = 0)=0;
...
public:
  ...
  // 用mvccmap_的最新版本更新map_
  void garbage_collect_fallback(const void *key) override{
  }

  std::tuple<MetaDataType *, void *> search_prev_fallback(const void *key,
    uint64_t version) override {};
  void fallback_insert(const void *key, const void *value,
              uint64_t version = 0) {};
...
/* parameter version is not used in Table. */
template <std::size_t N, class KeyType, class ValueType>
class ForeSightTable : public ITable {
public:
  using MetaDataType = std::atomic<uint64_t>;

  virtual ~ForeSightTable() override = default;

  ForeSightTable(std::size_t tableID, std::size_t partitionID)
      : tableID_(tableID), partitionID_(partitionID) {}
  ...
  std::tuple<MetaDataType *, void *> search_prev_fallback(const void *key,
                                                 uint64_t version) override {
    const auto &k = *static_cast<const KeyType *>(key);
    if(!mvccmap_.contains_key(k)){
      auto &v = map_[k];
      return std::make_tuple(&std::get<0>(v), &std::get<1>(v));
    }
    auto *v_ptr = mvccmap_.get_key_version_prev(k, version);
    CHECK(v_ptr != nullptr)
        << "key with version: " << version << " does not exist.";
    auto &v = *v_ptr;
    return std::make_tuple(&std::get<0>(v), &std::get<1>(v));
  ...
  void fallback_insert(const void *key, const void *value,
    ...
    const auto &k = *static_cast<const KeyType *>(key);
    const auto &v = *static_cast<const ValueType *>(value);
    bool ok = mvccmap_.contains_key_version(k, version);
    DCHECK(ok == false) << "version: " << version << " already exists.";
    // auto &row = mvccmap_.smart_insert_key_version_holder(k, version);
    auto &row = mvccmap_.insert_key_version_holder(k, version);
    std::get<0>(row).store(version);
    std::get<1>(row) = v;
  }
  ...
  void garbage_collect(const void *key) override {
    mvccmap_.remove_key(*static_cast<const KeyType *>(key));
  }

  // 用mvccmap_的最新版本更新map_
  void garbage_collect_fallback(const void *key) override{
    const auto &k = *static_cast<const KeyType *>(key);
    // mvccmap_.vacuum_key_keep_latest(k);
    // mvccmap_.set_latest_version(k);
    auto *v_ptr = mvccmap_.get_latest_value(k);
    if (v_ptr!=nullptr){ 
      auto &v = *v_ptr;
      auto &row = map_[k];
      std::get<0>(row).store(0);
      std::get<1>(row) = std::get<1>(v);
      mvccmap_.remove_key(k);
    }
  }

private:
  HashMap<N, KeyType, std::tuple<MetaDataType, ValueType>> map_;
  MVCCHashMap<N, KeyType, std::tuple<MetaDataType, ValueType>> mvccmap_;
  std::size_t tableID_;
  std::size_t partitionID_;
};
    ...
    static std::unique_ptr<ITable> create_table(const Context &context,
      ...
      if (context.mvcc) {
        return std::make_unique<MVCCTable<N, KeyType, ValueType>>(tableID,
                                                                  partitionID);
      }else if(context.protocol == "ForeSight") {
        return std::make_unique<ForeSightTable<N, KeyType, ValueType>>(tableID,
                                                              partitionID);
      }else {
