...
DEFINE_bool(if_local, false, "locality optimization for Bohm.");
...
DEFINE_int32(fsFB_lock_manager, 0,
             "# of lock manager in ForeSight's fallback mode.");
DEFINE_bool(ForeSight_calvin_fallback, false, "ForeSight fallback mode.");
...
#define SETUP_CONTEXT(context)                                                 \
  ...
  context.if_local = FLAGS_if_local;                                       \
  context.bohm_single_spin = FLAGS_bohm_single_spin;                           \
  context.ariaFB_lock_manager = FLAGS_ariaFB_lock_manager;                     \
  context.fsFB_lock_manager = FLAGS_fsFB_lock_manager;                     \
  context.calvin_fallback = FLAGS_ForeSight_calvin_fallback;                     \
...
//   CHECK((context.mvcc ^ ((context.protocol == "Bohm") || (context.protocol == "ForeSight"))) == 0)                    \
//       << "MVCC must be used in Bohm or ForeSight.";                                         \
