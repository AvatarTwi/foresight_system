...
import logging
from Structure.leaves.aspn_leaves.dimension_reduction import PCA_reduction
from Structure.leaves.aspn_leaves.utils import discretize_series
...
def create_multi_histogram_leaf(data, ds_context, scope, condition, alpha=False, discretize=False,
    ...
    if pca:
        ...
        # 计算总分箱数，检查是否会导致内存问题
        total_bins = np.prod([len(b) - 1 for b in breaks])
        max_safe_bins = 1e8  # 设置一个安全的最大分箱数 (100 million)
        
        if total_bins > max_safe_bins:
            logger.warning(f"Total number of bins ({total_bins}) exceeds safe limit ({max_safe_bins})")
            
            # 尝试自动减少每个维度的分箱数
            bin_reduction_factor = np.power(max_safe_bins / total_bins, 1.0 / len(breaks))
            logger.info(f"Reducing bin count per dimension by factor {bin_reduction_factor:.4f}")
            
            reduced_breaks = []
            for b in breaks:
                # 确保每个维度至少保留5个分箱
                new_count = max(5, int(len(b) * bin_reduction_factor))
                # 从原始的breaks中均匀选择新的breakpoints
                indices = np.linspace(0, len(b) - 1, new_count + 1, dtype=int)
                reduced_breaks.append(np.array([b[i] for i in indices]))
            
            breaks = reduced_breaks
            # 重新计算break_size
            break_size = [len(b) - 1 for b in breaks]
            logger.info(f"Reduced histogram size to {break_size}")
            
            # 重新计算总分箱数检查是否安全
            total_bins = np.prod(break_size)
            if total_bins > max_safe_bins:
                # 如果仍然太大，则强制使用PCA降维
                logger.warning(f"After reduction, bins ({total_bins}) still too large, forcing PCA reduction")
                pca, data = PCA_reduction(data, target_dimensions=min(5, data.shape[1]))
                ...
                for i in range(data.shape[1]):
                    ...
                    breaks.append(np.array(cont_break))
                    break_size.append(n_mcv+n_bins)
                logger.info(f"PCA-reduced histogram of size {break_size}")
        ...
        try:
            ...
            # 内存错误的紧急处理 - 创建极简版的直方图
            logger.error(f"Memory error creating histogram: {str(e)}")
            logger.info("Creating simplified histogram with minimal bins")
            
            # 每个维度只使用最小数量的bins (如2-3个)
            simplified_breaks = []
            for i in range(data.shape[1]):
                col_data = data[:, i]
                min_val, max_val = np.min(col_data), np.max(col_data)
                # 创建只有3个断点的简化分箱 (产生2个bin)
                simplified_breaks.append(np.array([min_val, (min_val + max_val)/2, max_val]))
            
            pdf, breaks = np.histogramdd(data, bins=simplified_breaks)
            pdf = pdf/np.sum(pdf)
            cdf = multidim_cumsum(pdf)
