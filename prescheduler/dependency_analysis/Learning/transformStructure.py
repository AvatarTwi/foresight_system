...
from Structure.nodes import Leaf, Joint, Independent, Decomposition, assign_ids, get_nodes_by_type, get_parents, get_topological_order
import logging
...
def Prune(node, contract_single_parents=True):
    v, err = is_valid(node)
    assert v, err
    nodes = get_nodes_by_type(node, (Independent, Joint))
    ...
    if contract_single_parents and isinstance(node, (Independent, Joint)) and len(node.children) == 1:
...
def SPN_Reshape(node, max_children=2):
    ...
    while len(nodes) > 0:
        ...
        for i in range(0, len(n.children), max_children):
            ...
            if len(children) > 1:
                if isinstance(n, Independent):
                    newChild = Independent()
                    for c in children:
