from Structure.nodes import Joint, Independent, Decomposition, get_nodes_by_type
import logging
...
def is_consistent(node):
    ...
    for prod_node in reversed(get_nodes_by_type(node, Independent)):
        nscope = set(prod_node.scope)

        if len(prod_node.children) == 0:
            return False, "Independent node %s has no children" % (prod_node.id)

        #if len(prod_node.condition) != 0:
            #logger.warning(f"Independent node {prod_node.id} has condition {prod_node.condition}")
            #return (False, "Independent node %s has condition" % (prod_node.id))
...
def check_factorize_node(node):
    ...
    """
    allchildscope = set()
    for fact_node in reversed(get_nodes_by_type(node, Decomposition)):
...
def is_complete(node):
    ...
    for sum_node in reversed(get_nodes_by_type(node, Joint)):
        nscope = set(sum_node.scope)
        ncondition = set(sum_node.condition)

        if len(sum_node.children) == 0:
            return False, "Joint node %s has no children" % (sum_node.id)
...
def is_valid(node, check_ids=True):
    ...
    for n in get_nodes_by_type(node):
        ...
        is_sum = isinstance(n, Joint)
        is_prod = isinstance(n, Independent)
        is_factorize = isinstance(n, Decomposition)
