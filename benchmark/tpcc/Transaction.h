...
// Specializations for different key types
template<>
std::size_t getKeyID(const warehouse::key& key) {
  std::string keyStr = "warehouse" + std::to_string(key.W_ID);
  return stringToInt64(keyStr);
}

template<>
std::size_t getKeyID(const district::key& key) {
  std::string keyStr = "district" + std::to_string(key.D_W_ID) + "_" + std::to_string(key.D_ID);
  return stringToInt64(keyStr);
}

template<>
std::size_t getKeyID(const customer::key& key) {
  std::string keyStr = "customer" + std::to_string(key.C_W_ID) + "_" + std::to_string(key.C_D_ID) + "_" + std::to_string(key.C_ID);
  return stringToInt64(keyStr);
}

template<>
std::size_t getKeyID(const item::key& key) {
  std::string keyStr = "item" + std::to_string(key.I_ID);
  return stringToInt64(keyStr);
}

template<>
std::size_t getKeyID(const stock::key& key) {
  std::string keyStr = "stock" + std::to_string(key.S_W_ID) + "_" + std::to_string(key.S_I_ID);
  return stringToInt64(keyStr);
}

template<>
std::size_t getKeyID(const customer_name_idx::key& key) {
  std::string keyStr = "customer_name_idx" + std::to_string(key.C_W_ID) + "_" + std::to_string(key.C_D_ID) + "_" + key.C_LAST.toString();
  return stringToInt64(keyStr);
}
...
public:  // 添加这一行
  ...
  TransactionResult execute(std::size_t worker_id) override {
    ...
    this->search_for_read(warehouseTableID, W_ID - 1, storage.warehouse_key,
                          storage.warehouse_value, getKeyID(storage.warehouse_key));
    ...
    this->search_for_update(districtTableID, W_ID - 1, storage.district_key,
                            storage.district_value, getKeyID(storage.district_key));
    ...
    this->search_for_read(customerTableID, W_ID - 1, storage.customer_key,
                          storage.customer_value, getKeyID(storage.customer_key));
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      this->search_local_index(itemTableID, 0, storage.item_keys[i],
                               storage.item_values[i], getKeyID(storage.item_keys[i]));
      ...
      this->search_for_update(stockTableID, OL_SUPPLY_W_ID - 1,
                              storage.stock_keys[i], storage.stock_values[i], getKeyID(storage.stock_keys[i]));
    }
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      this->update(stockTableID, OL_SUPPLY_W_ID - 1, storage.stock_keys[i],
                   storage.stock_values[i], getKeyID(storage.stock_keys[i]));
  ...
  TransactionResult execute(std::size_t worker_id) override {
    ...
    if (C_ID == 0) {
      ...
      this->search_for_read(customerNameIdxTableID, C_W_ID - 1,
                            storage.customer_name_idx_key,
                            storage.customer_name_idx_value, getKeyID(storage.customer_name_idx_key));
